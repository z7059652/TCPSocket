...
//typedef int(*lpCanWorkLoop)(int);
//lpCanWorkLoop lpcanworkLoop;
//lpCanWorkLoop lpdisconnect;
 ...
 *
 */
BOOL freerdp_connect_redirect(freerdp* instance)
{
	rdpRdp* rdp;
	BOOL status = TRUE;
...
BOOL freerdp_connect(freerdp* instance)
{
	if (UsingMode == _RDP_PROJECTOR)
		freerdp_connect_redirect(instance);
	else
	{
		rdpRdp* rdp;
		BOOL status = TRUE;

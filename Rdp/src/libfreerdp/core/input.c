...
void(*gmSendHIDData)(void *data, unsigned int size);
...
void SendHIDData(HIDType type, UINT Msg, UINT X, UINT Y)
{
	if (gmSendHIDData)
	{
		HIDDATASTRUCT senddata;
		senddata.type = type;
		senddata.Msg = Msg;
		senddata.Xpos = X;
		senddata.Ypos = Y;
		gmSendHIDData(&senddata, sizeof(HIDDATASTRUCT));
	}
}
...
BOOL freerdp_input_send_keyboard_event(rdpInput* input, UINT16 flags, UINT16 code)
{
	...
	{
		SendHIDData(HID_KEYBOARD, flags, code, 0);
		return TRUE;
	}
}

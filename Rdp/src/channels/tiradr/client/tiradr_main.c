...
#include "tiradr_main.h"

typedef struct _DEVICE_DRIVE_EXT DEVICE_DRIVE_EXT;
_declspec(dllexport) void(*gSendDRData)(void *data, unsigned int size);
 ...
 */
static UINT tirardpdr_send_device_list_announce_request(rdpdrPlugin* rdpdr, BOOL userLoggedOn);
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_send_device_list_remove_request(rdpdrPlugin* rdpdr, UINT32 count, UINT32 ids[])
{
	UINT32 i;
	wStream* s;
					...
					if (lpdb -> dbch_devicetype == DBT_DEVTYP_VOLUME)
					{
						...
						for (i = 0; i < 26; i++)
						{
							if (unitmask & 0x01)
							{
								...
								tirardpdr_send_device_list_announce_request(rdpdr, TRUE);
							}
							unitmask = unitmask >> 1;
						}
					}
							...
							if (unitmask & 0x01)
							{
								...
								for (j = 0; j < count; j++)
								{
									...
									if (device_ext->path[0] == drive_name_upper || device_ext->path[0] == drive_name_lower)
									{
										...
										ids[0] = keys[j];
										if ((error = tirardpdr_send_device_list_remove_request(rdpdr, 1, ids)))
										{
											// dont end on error, just report ?
											WLog_ERR(TAG, "tirardpdr_send_device_list_remove_request failed with error %lu!", error);
										}
										break;
									}
								}
...
cleanup:
	...
	return error ? error : tirardpdr_send_device_list_announce_request(rdpdr, TRUE);
}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_process_connect(rdpdrPlugin* rdpdr)
{
	UINT32 index;
	RDPDR_DEVICE* device;
...
static void tirardpdr_process_server_announce_request(rdpdrPlugin* rdpdr, wStream* s)
{
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_send_client_announce_reply(rdpdrPlugin* rdpdr)
{
	wStream* s;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_send_client_name_request(rdpdrPlugin* rdpdr)
{
	wStream* s;
...
static void tirardpdr_process_server_clientid_confirm(rdpdrPlugin* rdpdr, wStream* s)
{
	UINT16 versionMajor;
	UINT16 versionMinor;
	UINT32 clientID;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_process_irp(rdpdrPlugin* rdpdr, wStream* s)
{
	IRP* irp;
	UINT error = CHANNEL_RC_OK;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_process_receive(rdpdrPlugin* rdpdr, wStream* s)
{
	UINT16 component;
	UINT16 packetId;
	UINT32 deviceId;
	...
	if (component == RDPDR_CTYP_CORE)
	{
		switch (packetId)
		{
			case PAKID_CORE_SERVER_ANNOUNCE:
				if ((error = tirardpdr_send_client_announce_reply(rdpdr)))
				{
					WLog_ERR(TAG, "tirardpdr_send_client_announce_reply failed with error %lu", error);
					return error;
				}
				if ((error = tirardpdr_send_client_name_request(rdpdr)))
				{
					WLog_ERR(TAG, "tirardpdr_send_client_name_request failed with error %lu", error);
					return error;
				}
				break;
			...
			case PAKID_CORE_CLIENTID_CONFIRM:
				if ((error = tirardpdr_send_device_list_announce_request(rdpdr, FALSE)))
				{
					WLog_ERR(TAG, "tirardpdr_send_device_list_announce_request failed with error %lu", error);
					return error;
				}
				break;

			case PAKID_CORE_USER_LOGGEDON:
				if ((error = tirardpdr_send_device_list_announce_request(rdpdr, TRUE)))
				{
					WLog_ERR(TAG, "tirardpdr_send_device_list_announce_request failed with error %lu", error);
					return error;
				}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
UINT tirardpdr_add_init_handle_data(void* pInitHandle, void* pUserData)
{
	if (!g_InitHandles)
	{
...
void* tirardpdr_get_init_handle_data(void* pInitHandle)
{
	void* pUserData = NULL;
...
void tirardpdr_remove_init_handle_data(void* pInitHandle)
{
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
UINT tirardpdr_add_open_handle_data(DWORD openHandle, void* pUserData)
{
...
void* tirardpdr_get_open_handle_data(DWORD openHandle)
{
	void* pUserData = NULL;
	void* pOpenHandle = (void*) (size_t) openHandle;
...
void tirardpdr_remove_open_handle_data(DWORD openHandle)
{
	void* pOpenHandle = (void*) (size_t) openHandle;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
UINT tirardpdr_send(rdpdrPlugin* rdpdr, wStream* s)
{
	UINT status;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_virtual_channel_event_data_received(rdpdrPlugin* rdpdr,
		void* pData, UINT32 dataLength, UINT32 totalLength, UINT32 dataFlags)
{
	...
	if (dataFlags & CHANNEL_FLAG_LAST)
	{
		...
		{
			WLog_ERR(TAG, "tirardpdr_virtual_channel_event_data_received: read error");
			return ERROR_INTERNAL_ERROR;
		}
...
static VOID VCAPITYPE tirardpdr_virtual_channel_open_event(DWORD openHandle, UINT event,
		LPVOID pData, UINT32 dataLength, UINT32 totalLength, UINT32 dataFlags)
{
	rdpdrPlugin* rdpdr;
	UINT error = CHANNEL_RC_OK;

	rdpdr = (rdpdrPlugin*) tirardpdr_get_open_handle_data(openHandle);

	if (!rdpdr)
	{
		WLog_ERR(TAG,  "tirardpdr_virtual_channel_open_event: error no match");
		return;
	}
	...
	switch (event)
	{
		case CHANNEL_EVENT_DATA_RECEIVED:
			if ((error  = tirardpdr_virtual_channel_event_data_received(rdpdr, pData, dataLength, totalLength, dataFlags)))
				WLog_ERR(TAG,  "tirardpdr_virtual_channel_event_data_received failed with error %lu!", error );
			break;
	...
	if (error && rdpdr->rdpcontext)
		setChannelError(rdpdr->rdpcontext, error, "tirardpdr_virtual_channel_open_event reported an error");

	return;
}

static void* tirardpdr_virtual_channel_client_threadt(void* arg)
{
	wStream* data;
	wMessage message;
	...
	if ((error = tirardpdr_process_connect(rdpdr)))
	{
		WLog_ERR(TAG, "tirardpdr_process_connect failed with error %lu!", error);
		if (rdpdr->rdpcontext)
			setChannelError(rdpdr->rdpcontext, error, "tirardpdr_virtual_channel_client_threadt reported an error");
		ExitThread((DWORD) error);
		return NULL;
	}
	...
	while (1)
	{
		...
		if (MessageQueue_Peek(rdpdr->queue, &message, TRUE))
		{
			...
			if (message.id == 0)
			{
				...
				gSendDRData(data->pointer, data->length);
				if ((error = tirardpdr_process_receive(rdpdr, data)))
				{
					WLog_ERR(TAG, "tirardpdr_process_receive failed with error %lu!", error);
					if (rdpdr->rdpcontext)
						setChannelError(rdpdr->rdpcontext, error, "tirardpdr_virtual_channel_client_threadt reported an error");
					ExitThread((DWORD) error);
					return NULL;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_virtual_channel_event_connected(rdpdrPlugin* rdpdr, LPVOID pData, UINT32 dataLength)
{
	UINT32 status;
	UINT error;

	status = rdpdr->channelEntryPoints.pVirtualChannelOpen(rdpdr->InitHandle,
		&rdpdr->OpenHandle, rdpdr->channelDef.name, tirardpdr_virtual_channel_open_event);
    ...
    if ((error = tirardpdr_add_open_handle_data(rdpdr->OpenHandle, rdpdr)))
	{
		WLog_ERR(TAG, "tirardpdr_add_open_handle_data failed with error %lu!", error);
		return error;
	}
	...
	if (!(rdpdr->thread = CreateThread(NULL, 0,
			(LPTHREAD_START_ROUTINE) tirardpdr_virtual_channel_client_threadt, (void*) rdpdr, 0, NULL)))
	{
		WLog_ERR(TAG, "CreateThread failed!");
		return ERROR_INTERNAL_ERROR;
	}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT tirardpdr_virtual_channel_event_disconnected(rdpdrPlugin* rdpdr)
{
	UINT error;
	...
	tirardpdr_remove_open_handle_data(rdpdr->OpenHandle);
	return error;
}

static void tirardpdr_virtual_channel_event_terminated(rdpdrPlugin* rdpdr)
{
	tirardpdr_remove_init_handle_data(rdpdr->InitHandle);

	free(rdpdr);
}

static VOID VCAPITYPE tirardpdr_virtual_channel_init_event(LPVOID pInitHandle, UINT event, LPVOID pData, UINT dataLength)
{
	rdpdrPlugin* rdpdr;
	UINT error = CHANNEL_RC_OK;

	rdpdr = (rdpdrPlugin*) tirardpdr_get_init_handle_data(pInitHandle);
	...
	switch (event)
	{
		...
		case CHANNEL_EVENT_CONNECTED:
			if ((error = tirardpdr_virtual_channel_event_connected(rdpdr, pData, dataLength)))
				WLog_ERR(TAG, "tirardpdr_virtual_channel_event_connected failed with error %lu!", error);
			break;

		case CHANNEL_EVENT_DISCONNECTED:
			if ((error = tirardpdr_virtual_channel_event_disconnected(rdpdr)))
				WLog_ERR(TAG, "tirardpdr_virtual_channel_event_disconnected failed with error %lu!", error);
			break;

		case CHANNEL_EVENT_TERMINATED:
			tirardpdr_virtual_channel_event_terminated(rdpdr);
			break;
		default:
	...
	if (error && rdpdr->rdpcontext)
		setChannelError(rdpdr->rdpcontext, error, "tirardpdr_virtual_channel_init_event reported an error");
	return;
}

/* rdpdr is always built-in */
#define VirtualChannelEntry	tiradr_VirtualChannelEntry
...
BOOL VCAPITYPE VirtualChannelEntry(PCHANNEL_ENTRY_POINTS pEntryPoints)
{
	...
	rc = rdpdr->channelEntryPoints.pVirtualChannelInit(&rdpdr->InitHandle,
		&rdpdr->channelDef, 1, VIRTUAL_CHANNEL_VERSION_WIN2000, tirardpdr_virtual_channel_init_event);
	...
	if ((rc = tirardpdr_add_init_handle_data(rdpdr->InitHandle, (void*) rdpdr)))
	{
		WLog_ERR(TAG, "tirardpdr_add_init_handle_data failed with error %lu!", rc);
		free(rdpdr);
		return FALSE;
	}

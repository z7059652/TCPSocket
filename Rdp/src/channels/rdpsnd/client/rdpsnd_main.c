...
_declspec(dllexport) void(*gmAudioPlaybackRedirectData)(void *data, unsigned int size);
_declspec(dllexport) void(*gSendAudioFormatInfo)(void *data, unsigned int size);
__declspec(dllexport) rdpsndPlugin* g_rdpsndPlugin = NULL;
...
UINT rdpsnd_send_client_audio_formats(rdpsndPlugin* rdpsnd)
{
	...
	gSendAudioFormatInfo(pdu->buffer,pdu->length);
...
BOOL VCAPITYPE VirtualChannelEntry(PCHANNEL_ENTRY_POINTS pEntryPoints)
{
	...
	if (g_rdpsndPlugin == NULL)
		g_rdpsndPlugin = rdpsnd;

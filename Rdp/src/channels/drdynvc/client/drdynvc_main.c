...
#include "../../client/rdpSourceMode.h"

#define TAG CHANNELS_TAG("drdynvc.client")

static void dvcman_channel_free(void* channel);
extern  __declspec(dllexport) drdynvcPlugin* g_drdynvc = NULL;
/**
...
//	WLog_ERR(TAG, "order_recv: Cmd=0x%x, Sp=%d cbChId=%d, ChannelId=%d", Cmd, Sp, cbChId);
...
static int SetupVideoVirtualChannel(void* arg)
{
	//setup channel data should be received from server or middle redirector
	int value;
	int Cmd;
	int Sp;
	int cbChId;
	...
	BYTE tmp[12] = { 0x50, 0x00, 0x03, 0x00, 0x33, 0x33, 0x11, 0x11, 0x3d, 0x0a, 0xa7, 0x04 };
	wStream *s1 = Stream_New(tmp, 12);
	Stream_Read_UINT8(s1, value);
	Cmd = (value & 0xf0) >> 4;
	Sp = (value & 0x0c) >> 2;
	cbChId = (value & 0x03) >> 0;
	error = drdynvc_process_capability_request(drdynvc, Sp, cbChId, s1);
	if (error) return ERROR_INTERNAL_ERROR;
	Stream_Free(s1, FALSE);
	BYTE  tmp1[36] = { 0x18, 0x06, 0x4d, 0x69, 0x63, 0x72, 0x6f, 0x73, 0x6f, 0x66, 0x74, 0x3a, 0x3a, 0x57, 0x69, 0x6e, 0x64,
		0x6f, 0x77, 0x73, 0x3a, 0x3a, 0x52, 0x44, 0x53, 0x3a, 0x3a, 0x47, 0x72, 0x61, 0x70, 0x68, 0x69, 0x63, 0x73, 0x00 };
	wStream *s = Stream_New(tmp1, 36);
	Stream_Read_UINT8(s, value);
	Cmd = (value & 0xf0) >> 4;
	Sp = (value & 0x0c) >> 2;
	cbChId = (value & 0x03) >> 0;
	error = drdynvc_process_create_request(drdynvc, Sp, cbChId, s);
	if (error) return ERROR_INTERNAL_ERROR;
	Stream_Free(s, FALSE);
	return error;
}
static void* drdynvc_virtual_channel_client_thread(void* arg)
{
	...
	if (UsingMode == _RDP_PROJECTOR)
		error = SetupVideoVirtualChannel(arg);

...
typedef int(*lpCanWorkLoop)();
lpCanWorkLoop  _declspec(dllexport) lpcanworkLoop;


__declspec(dllexport) BOOL g_IsAlive;
...
_declspec (dllexport)  rdpInput* gInPut;
...
DWORD WINAPI wf_client_thread_redirect(rdpContext* context)
{
	MSG msg;
	int width;
	int height;
	...
	g_IsAlive = TRUE;
	while (msg_ret = GetMessage(&msg, NULL, 0, 0))
	{
...
int wfreerdp_client_start(rdpContext* context)
{
	...
	{
		wfc->thread = CreateThread(NULL, 0, wf_client_thread_redirect, context, 0, &wfc->mainThreadId);
		while (!IsWindowVisible(wfc->hwnd))
		{
			Sleep(100);
		}
		if (wfc->settings->GrabKeyboard)
		{
...
_declspec(dllexport) void(*gRdpDisconnect)();
